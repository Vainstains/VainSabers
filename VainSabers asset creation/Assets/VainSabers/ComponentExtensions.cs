    ...
    public static class ComponentInjectionExtension
    {
        ...
        private class InjectFromSelfEntry : IInjectionEntry
        {
            ...
            public bool TryInject(object instance)
            {
                ...
                {
                    if (wasAdded)
                        Debug.Log($"Injecting newly instantiated {m_fieldInfo.FieldType.Name} from \"{comp.gameObject.name}\" (self)");
                    else
                        Debug.Log($"Injecting existing {m_fieldInfo.FieldType.Name} from \"{comp.gameObject.name}\" (self)");
