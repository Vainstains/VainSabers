...
internal class SaberSweepGenerator : MonoBehaviour
{
    ...
    public void Init(Transform saber, SaberSweepData sweepData, int sampleCount, float lookBackTime, LegacySaberProfile profile)
    {
        m_saber = saber;
        ...
        m_saberMaterial = new Material(VainSabersAssets.SaberShader);

        m_profile = profile;

        m_interpolationBaseOffset = m_profile.Vertices[0].Position.x;
    ...
    public void LateUpdate()
    {
        ...
        InterpolateData(currentPos, currentDir, -currentTipVel, oldPos, oldDir, -oldTipVel, -currentTipVel);
    ...
    private void UpdateRollingTipSpeed(float rawTipSpeed, float difference)
    {
        ...
        if (m_rollingTipSpeed > 0.15f && difference > 0.02f)
        {
    ...
    private void InterpolateData(Vector3 posA, Vector3 dirA, Vector3 dirTangentA, Vector3 posB, Vector3 dirB, Vector3 dirTangentB, Vector3 fallbackTangent)
    {
        ...
        float posDifference = (posB - posA).magnitude;
        float dirDifference = (dirB - dirA).magnitude;
        if ((posDifference * dirDifference) < 0.2f)
        {
            Vector3 avgTangent = (dirTangentA + dirTangentB) / 2f;
            dirTangentA = dirTangentB = avgTangent;
        }
